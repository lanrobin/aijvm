...
void register_java_io_PrintStream(NativeMethodRegistry& registry) {
    ...
    registry.register_method(
        ...
        []([[maybe_unused]] VMContext& ctx) {
            auto* str_obj = static_cast<JObject*>(ctx.frame.pop_ref());  // String arg
            (void)ctx.frame.pop_ref();  // 'this' (PrintStream instance)
    ...
    registry.register_method(
        NativeMethodRegistry::make_key("java/io/PrintStream", "print",
            "(Ljava/lang/String;)V"),
        []([[maybe_unused]] VMContext& ctx) {
            auto* str_obj = static_cast<JObject*>(ctx.frame.pop_ref());
            (void)ctx.frame.pop_ref();  // 'this'
            std::cout << extract_java_string(str_obj);
        });
